...
def read_root(res: Response, db: Session = Depends(get_db), name: Optional[str] = None, city: Optional[str] = None):
    if name:
        ...
        print(user)
        if city:
            weather = requests.get("https://api.weatherapi.com/v1/forecast.json?key=e9c23a3c8b4040488e7112737212006&q=" + city + "&days=7&aqi=no&alerts=no").json()
            return {
                'location': weather['location']['country'] + ' - ' + weather['location']['region'] + ' - ' + weather['location']['name'],
                'today': {
